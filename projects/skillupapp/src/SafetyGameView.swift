...
struct SafetyGameView: View {
    @EnvironmentObject var store: ProfilesStore

    @State private var questions = SampleData.safety.shuffled()
    ...
    func color(for opt: String) -> Color {
        guard let s = selected else { return Theme.warn }
