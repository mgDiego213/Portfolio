# V2: Tokenization + Lemmatization (NLTK) + classifier
import os, re
import pandas as pd
...
def main():
    ...
    proba = model.predict_proba(comments_vec)[:, 1]
    label = (proba >= 0.5).astype(int)

    for c, p, lab in zip(comments, proba, label):
        print("\nComment:", c)
        print("P(positive) =", float(p))
        print("Prediction  =", "positive" if lab == 1 else "negative")
